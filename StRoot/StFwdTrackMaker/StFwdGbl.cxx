...
StFwdGbl::StFwdGbl() :
genfit::AbsFitter(), m_milleFileName("millefile.dat"), m_gblInternalIterations("THC"), m_pValueCut(0.), m_minNdf(1),
m_chi2Cut(0.),
...
void StFwdGbl::processTrackWithRep(genfit::Track* trk, const genfit::AbsTrackRep* rep, bool resortHits)
{
  ...
  for (int ipoint_meas = 0; ipoint_meas < npoints_meas; ipoint_meas++) {
    ...
    genfit::PlanarMeasurement* measPlanar = dynamic_cast<genfit::PlanarMeasurement*>(point_meas->getRawMeasurement(0));
    ...
    if (!skipMeasurement) {
      ...
      int label;
      if(sensorId < 1000) label = sensorId * 10;
      if(sensorId >= 1000)
      {
        if((sensorId - 1000)%3 == 0)      label = 1000 + (sensorId-1000) * 10;
        else if((sensorId - 1000)%3 == 1) label = 1000 + (sensorId-1000) * 10;
        else if((sensorId - 1000)%3 == 2) label = 1000 + (sensorId-1001) * 10;
      }

      cout << "SensorID = " << sensorId << "     label = " << label << endl;
      ...
      derGlobal(0, 0) = 1.0;                    // du/du
      derGlobal(0, 1) = 0.0;                    // du/dv
      derGlobal(0, 2) = - uSlope;               // du/dw
      derGlobal(0, 3) = vPos * uSlope;          // du/d(alpha)
      derGlobal(0, 4) = -uPos * uSlope;         // du/d(beta)
      derGlobal(0, 5) = -vPos;                   // du/d(gamma)
         
      derGlobal(1, 0) = 0.0;                    // dv/du
      derGlobal(1, 1) = 1.0;                    // dv/dv
      derGlobal(1, 2) = - vSlope;               // dv/dw
      derGlobal(1, 3) = vPos * vSlope;          // dv/d(alpha)
      derGlobal(1, 4) = -uPos * vSlope;         // dv/d(beta)
      derGlobal(1, 5) = uPos;                  // dv/d(gamma)
